...
type TweakOption = string | number | boolean | { label: string; value: string | number | boolean };
...
export function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  const maxLen: number = options.reduce((m: number, o) => Math.max(m, labelLen(o)), 0);
  const maxAllowed: Record<number, number> = { 2: 16, 3: 10 };
  const fitsAsSegments = maxLen <= (maxAllowed[options.length] ?? 0);

  if (!fitsAsSegments) {
    const resolve = (s: string): string | number | boolean => {
