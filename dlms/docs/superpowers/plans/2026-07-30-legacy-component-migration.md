...
| **PCBA-B** (控制板) | `pcba_b_sn`, `pcba_b_hw_rev`, `pcba_b_bom_rev`, `pcba_b_fw_ver` | all nullable; the schema comment warns `pcba_b_sn` **may contain prose**, e.g. `"No wifi version"` | a serialized `component_unit` (`pcba_b`) + one open installation, when a serial is present |
...
**The prose decision.** `pcba_b_sn` may hold `"No wifi version"`, which is a statement that there is no board, not a serial. Detecting that reliably is guesswork, and the house rule is that a migration which guesses is a migration that silently corrupts a fleet (`migrate_demo.ts:20-22`). So the mapper does not judge: it carries every present value verbatim into a unit and sets `needs_split = true` on anything that is not a single clean serial. `needs_split` is the only flag `component_unit` has, and the admin cleanup queue works off it — so the widening from "holds a range" to "this serial needs human eyes" is deliberate. Say so in the field's comment, and make the reconciliation report list them, so the queue is visible rather than implied.
