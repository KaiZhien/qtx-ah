...
- **AAL2 enforcement at the server-action/capability layer.** This build gates *page rendering* via the platform layout; it does NOT gate direct server-action dispatches (Next.js layouts don't run for actions — see §7). Closing that was the **immediate follow-up** (`assertAal2()` in `requireActor` or the privileged actions), tracked separately, so the capability layer is genuinely AAL2-only. **CLOSED 2026-07-20** — shipped as `requireAal2Actor` (a new wrapper, not a change to `requireActor`); every `(platform)` `'use server'` module now calls it.
