...
- **Post-login landing.** `/login` still redirects to `/legacy`. An MFA-required user therefore reaches the gate the moment they open a platform route (where the powers live), not immediately at login. Enforcement is sound (no platform capability is reachable at AAL1); redirecting platform users straight to `/` after login is a noted follow-up, not part of this build.
