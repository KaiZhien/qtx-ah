          ...
          <CardContent>
            <div className="flex flex-wrap gap-1 mt-1">
              {Object.entries(stats.byStatus).map(([s, n]) => (
                <Link key={s} href={`/legacy/devices?status=${encodeURIComponent(s)}`}>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-1 mt-1">
              {Object.entries(stats.byCustomer).slice(0, 5).map(([c, n]) => (
                <Link key={c} href={`/legacy/devices?customer=${encodeURIComponent(c)}`}>
            ...
            {recentActivity.length === 0 ? (
              ...
              recentActivity.map((entry) => (
                <div key={entry.id} className="flex items-start gap-3 text-sm border-b last:border-0 pb-2 last:pb-0">
                  ...
                  {entry.row_id ? (
                    <Link href={`/legacy/devices/${entry.row_id}`} className="font-mono text-xs hover:underline truncate">
