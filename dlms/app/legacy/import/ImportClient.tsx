    ...
    <div className="space-y-6 max-w-2xl w-full">
      ...
      {result && (
        <Alert>
          <AlertDescription>
            <p>
              Import complete: {result.imported} imported · {result.skippedDuplicate} duplicates skipped · {result.skippedInvalid} invalid · {result.failed.length} failed.
            </p>
            {result.failed.length > 0 && (
              <ul className="mt-2 list-disc pl-5 text-xs">
                {result.failed.map((f) => (
                  <li key={f.rowIndex}>Row {f.rowIndex}: {f.error}</li>
                ))}
              </ul>
            )}
          </AlertDescription>
