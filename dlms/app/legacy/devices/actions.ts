...
export async function createDeviceBatchAction(input: DeviceInput): Promise<never> {
  ...
  if (units.length === 1) {
    ...
    redirect(`/legacy/devices/${device.id}`)
  }
  ...
  revalidatePath('/legacy/devices')
  redirect(`/legacy/devices?batchCreated=${units.length}`)
}
...
export async function updateDeviceAction(id: string, input: Partial<DeviceInput>, version: number) {
  ...
  revalidatePath(`/legacy/devices/${id}`)
  return device
}
...
export async function assignDeviceAction(
  ...
  try {
    ...
    revalidatePath(`/legacy/devices/${deviceId}`)
    return { ok: true }
  } catch (e: unknown) {
