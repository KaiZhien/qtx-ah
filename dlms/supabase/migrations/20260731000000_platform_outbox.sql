...
COMMENT ON COLUMN outbox.aggregate_type IS
  'Left unconstrained on purpose. A CHECK listing today''s single value (''device'') would make every future aggregate a schema migration, which defeats the point of a generic outbox. The drain dispatches on this value together with event_type, and a combination it does not handle is RECORDED as a failed attempt — attempts incremented, last_error set, processed_at left NULL — never silently skipped. Skipping is the tempting behaviour and the wrong one: an unrecognised row would stay unprocessed, invisible, and re-claimed by every drain forever, with attempts never rising to park it. Recording makes the disagreement legible in the table and self-limiting, because the drain''s attempts cap eventually parks the row for a human. The contract that implies is SINGLE-CONSUMER: every row here is owed to the one drain, so a second consumer cannot simply ignore the first''s rows — each would record the other''s as failures. Adding one means an explicit ownership split (a dispatch registry, or a consumer column on this table), not a pair of mutually-ignoring readers.';
COMMENT ON COLUMN outbox.event_type IS
  'Left unconstrained, same reasoning as aggregate_type — including that an unhandled value is recorded as a failed attempt rather than skipped. ''device_status_changed'' is the only producer today.';
