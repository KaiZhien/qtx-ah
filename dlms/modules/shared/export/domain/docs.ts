...
export function buildRelationshipsMd(
  ...
  return [
    ...
    '- **`variant_bom_line` carries effectivity history**, so it holds one row per',
    '  (variant, component type, *effectivity window*) — NOT one row per component',
    '  type. The BOM in force today is the rows where `effective_to_date IS NULL AND',
    '  effective_to_serial IS NULL`; everything else is a superseded revision, with',
