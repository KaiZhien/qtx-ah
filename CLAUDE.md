...
- **DLMS security model**: all services use the service-role client, so **RLS is bypassed at runtime — the real boundary is `can()` in `lib/auth/permissions.ts`, enforced in pages, server actions, AND services** (belt-and-suspenders; RLS is defense-in-depth for direct PostgREST). Every status write must pass `isValidTransition` (enforced inside `updateDevice`); `allowedNextStatuses` fails closed. Pure logic lives in `lib/domain/` (no I/O, unit-tested); house rule: flat selects + JS reduce, no DB views/RPC.
- **DLMS audit**: `fn_audit` is shape-agnostic and attached to all mutable tables; `audit_log.row_id`/`new_values` are nullable. The edge function `device-api` writes as system actor `11111111-1111-1111-1111-111111111111`.
